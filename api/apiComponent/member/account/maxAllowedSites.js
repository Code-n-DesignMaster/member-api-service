...
module.exports = (components) =>
  (accessToken, customAccountSubscriptions = null, customUrl = null, userId = null) => {
          ...
          accountSubscriptions.map(accountSubscription => {
            if (accountSubscription.status === 'active' || accountSubscription.status === 'renewal_due') {
              ...
              if (accountSubscription.features && !_.isEmpty(accountSubscription.features)) {
                sitesFeature = accountSubscription.features.find(feature => feature.technicalName === 'sites');
                if (sitesFeature) {
              ...
              if (accountSubscription.addons && !_.isEmpty(accountSubscription.addons)) {
                sitesAddons = accountSubscription.addons.filter(addon => addon.feature && addon.feature.technicalName === 'sites' && (addon.status === 'active' || addon.status === 'renewal_due'));
