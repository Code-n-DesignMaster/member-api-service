          ...
          accountSubscriptions.map(accountSubscription => {
            if (accountSubscription.status === 'active' || accountSubscription.status === 'renewal_due') {
              ...
              if (accountSubscription.features && !_.isEmpty(accountSubscription.features)) {
                sitesFeature = accountSubscription.features.find(feature => feature.technicalName === 'advertisement');
                if (sitesFeature) {
                  showAdvertisement = showAdvertisement || (sitesFeature.featureValue === 'true' || sitesFeature.featureValue === true);
                }
              }
            }
          });

          resolve(showAdvertisement);
        })
        .catch(reject);
    });
  };
