          ...
          accountSubscriptions.map(accountSubscription => {
            if (accountSubscription.status === 'active' || accountSubscription.status === 'renewal_due') {
              ...
              if (accountSubscription.features && !_.isEmpty(accountSubscription.features)) {
                sitesFeature = accountSubscription.features.find(feature => feature.technicalName === 'enableBlocks');
                if (sitesFeature) {
                  enableBlocks = enableBlocks || (sitesFeature.featureValue === 'true' || sitesFeature.featureValue === true);
                }
              }
            }
          });

          resolve(enableBlocks);
        })
        .catch(reject);
    });
  };
